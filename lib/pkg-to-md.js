...
var optionToSectionWrite = { // TODO rename MD functions like options?
    tests: 'tests',
    inst: 'installation',
    staff: 'staff',
    lic: 'license',
...
/**
 * Collects all dependency types.
 * @type {string[]}
 * @private
 * @see {@link #runOrderedSectionsWrite}
 */
...
/**
 * Writes the orderable sections according to the order of defined `options`.
 * @param {Md} md
 * @param {string[]} orderedOptions
 * @returns {Thenable<U>|*|{anyOf}}
 */
function runOrderedSectionsWrite(md, orderedOptions) {
    return Promise.each(orderedOptions, function (option, index) {
        if (option === 'deps') {
            return Promise.each(DEPENDENCY_TYPES, function (depType) {
                return md.dependencies(depType);
            });
        } else {
            return md[optionToSectionWrite[option]]();
        }
    })
        .then(function () {
            return md;
 ...
 * @returns {Promise} -
 */
PkgToMd.prototype.writeMarkdown = function (options, sectionOrder) {
    var self = this;
    var pkgReader = new PkgReader(self.logger);
    return pkgReader.read(options.pkg) // reads and normalizes pkg!
        .then(function (pkg) {
            //self.logger.debug('PACKAGE::: ' + JSON.stringify(pkg, null, 4));
            return new Md(pkg, options, self.logger);
        })
        .then(function (md) {
            return md.title(options.ver);
        })
        .then(function (md) {
            return md.description();
        })
        .then(function (md) {
            return runOrderedSectionsWrite(md, sectionOrder)
        })

        // .then(function (md) {
        //     return md[optionToSectionWrite.inst]();//md.installation();
        // })
        // .then(function (md) {
        ...
        .then(function (md) {
            ...
            return mdWriter.write(md, options.md, options.force);
        });
    // .catch(function (err) {
